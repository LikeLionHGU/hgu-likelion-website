...
import { FilterButton, FilterGroup } from '../components/Project.styles';
import {
  ProfileNameRow,
  ProfileOverlay,
  ProfilePhotoButton,
...
export default function Team({ members = teamMembers }: TeamProps) {
  ...
  const track =
    teamTracks.find((item) => item.toLowerCase() === requestedTrack?.toLowerCase()) || 'OPS';
  ...
  return (
    <>
      <Helmet>
        <title>팀 | 멋쟁이사자처럼 한동대</title>
      </Helmet>
      {isMobile && selectedMember ? (
        <TeamProfileMobile
          member={selectedMember}
          hasMultipleMembers={profiles.length > 1}
          onMove={(direction) => {
        ...
        <TeamPage component="main">
          <TeamContent>
            <Typography variant="teamPageTitle" component="h1">
              TEAM.
              <Box
                component="span"
                sx={{
            ...
            <TeamLayout>
                  ...
                  <Box component="div" sx={{ display: 'grid', gap: '15px', justifyItems: 'start' }}>
                    {teamGenerations.map((item) => (
                      <FilterButton
                        ...
                        onClick={() => selectFilter(item, track)}
                      >
                        {item}th
                      </FilterButton>
                    ))}
                  ...
                  <Box component="div" sx={{ display: 'grid', gap: '15px', justifyItems: 'start' }}>
                    {teamTracks.map((item) => (
                      <FilterButton
                        key={item}
                        type="button"
                        aria-current={track === item ? 'true' : undefined}
                        onClick={() => selectFilter(generation, item)}
                      >
                        {item}
                      </FilterButton>
                    ))}
              ...
              <section aria-label={`${generation}기 ${track} 팀원`}>
                <TeamMobileTabs
                  value={track}
                  onChange={(_, value: TeamTrack) => selectFilter(generation, value)}
                  variant="scrollable"
                  scrollButtons={false}
                  aria-label="팀 트랙 선택"
                >
                  {teamTracks.map((item) => (
                    <TeamMobileTab key={item} value={item} label={item} />
                  ))}
                </TeamMobileTabs>
                <Typography
                  variant="titleSmall"
                ...
                <TeamCards>
                  {profiles.map((member) => (
                    <TeamCard component="article" key={member.id}>
                      <ProfilePhotoButton
                        type="button"
                        aria-label={`${member.name} ${member.role} 프로필 보기`}
                        aria-haspopup={isMobile ? undefined : 'dialog'}
                        onClick={() => selectMember(member)}
                      >
                        <img src={member.photo} alt={`${member.name} 프로필 사진`} loading="lazy" />
                        <ProfileOverlay data-profile-overlay>
                          <span>View Profile</span>
                        </ProfileOverlay>
                      </ProfilePhotoButton>
                      <Box component="div" sx={{ display: 'grid', gap: '5px' }}>
                        <ProfileNameRow>
                          <Typography variant="profileName" component="h3">
                            {member.name}
                          </Typography>
                          <ProfileRole
                            label={member.role}
                            variant="outlined"
                            isStudent={member.role === '아기사자'}
                          />
                        </ProfileNameRow>
                        <Typography
                          component="p"
                          ...
                          sx={{ color: 'site.muted' }}
                        >
                          {member.major} {member.admissionYear}
                        </Typography>
                      </Box>
                    </TeamCard>
                  ))}
                  {!profiles.length && (
                    <Typography
                      ...
                      variant="body1"
                      role="status"
                      sx={{ gridColumn: '1 / -1', p: '40px 0', color: 'site.muted' }}
                    >
                      등록된 팀원이 없습니다.
                    </Typography>
                  )}
