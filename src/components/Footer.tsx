...
const FooterContent = styled(Container, {
  ...
  ...(mainPage && {
    maxWidth: 1920,
    padding: '80px clamp(30px, 5.15625vw, 99px) 100px',
    gap: 28,
    [theme.breakpoints.up('sm')]: {
      paddingLeft: 'clamp(30px, 5.15625vw, 99px)',
      paddingRight: 'clamp(30px, 5.15625vw, 99px)',
    },
    ...
    <FooterRoot component="footer" mainPage={useFullWidthStyle}>
      <FooterContent mainPage={useFullWidthStyle} maxWidth={useFullWidthStyle ? false : 'lg'}>
        ...
        <Typography
          ...
          sx={{ display: 'flex', gap: 1, '& span': { fontFamily: 'inherit' } }}
        >
          <span>(주)멋쟁이사자처럼</span>|<span>한동대학교</span>
        </Typography>
        <Box component="div" data-footer-social sx={{ display: 'flex', gap: 1 }}>
          <IconButton
            color="inherit"
            component="a"
