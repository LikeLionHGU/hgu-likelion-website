...
import { HorizontalViewport, MAIN_ORANGE, SectionLayout, SectionTitle } from './Main.styles';
...
const ScheduleCanvas = styled(Box)(({ theme }) => ({
  position: 'relative', width: '100%', minWidth: 760, height: 463,
  [theme.breakpoints.down('md')]: { width: 825, minWidth: 825, height: 278 },
}));

const Seasons = styled(Box)(({ theme }) => ({
  display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: '4.5%',
...
const Season = styled(Box)(({ theme }) => ({
  ...
  '& + &::before': {
    content: '""', position: 'absolute', top: 45, left: 10, width: 1, height: 417,
...
const SeasonArrow = styled('img')(({ theme }) => ({
  display: 'block', width: '100%', height: 'clamp(12px, 1.041667vw, 20px)',
  [theme.breakpoints.down('md')]: { height: 12 },
}));

const SeasonText = styled(Box)(({ theme }) => ({
  margin: '30px 0 0 40px', display: 'grid', gap: 5,
  [theme.breakpoints.down('md')]: { margin: '18px 0 0 24px', gap: 4 },
}));

const SeasonTitle = styled(Typography)(({ theme }) => ({
  margin: 0,
  fontSize: 'clamp(18px, 1.25vw, 24px)', fontWeight: 700, color: MAIN_ORANGE, lineHeight: 1.2,
  [theme.breakpoints.down('md')]: { fontSize: 16 },
}));

const SeasonDescription = styled(Typography)(({ theme }) => ({
  fontSize: 'clamp(16px, 1.145833vw, 22px)', fontWeight: 500, color: '#C4C4C4', lineHeight: 1.2,
  [theme.breakpoints.down('md')]: { fontSize: 14 },
}));

const ScheduleEvent = styled(Typography)(({ theme }) => ({
  position: 'absolute', display: 'flex', alignItems: 'center', justifyContent: 'center',
  height: 40, border: `1px solid ${MAIN_ORANGE}`, borderRadius: 20,
  background: 'rgba(243,114,15,0.5)', color: theme.palette.common.white,
  fontSize: 'clamp(14px, 1.041667vw, 20px)', fontWeight: 600, lineHeight: 1.2, whiteSpace: 'nowrap',
  ...
  '&[data-event="union"]': { left: '72.7%', top: 375, width: '27.3%', fontSize: 'clamp(12px, 1.041667vw, 20px)' },
  '&[data-event="hackathon"], &[data-event="ideathon"], &[data-event="union"]': {
    background: 'rgba(235,89,0,0.5)', borderColor: '#EB5900',
  },
  [theme.breakpoints.down('md')]: {
    height: 26, fontSize: 14, borderRadius: 13,
...
export default function MainSchedule() {
  return (
    <SectionLayout>
      <SectionTitle as="h2" id="main-schedule-title">Main schedule</SectionTitle>
          ...
          <Seasons>
            {seasons.map((season) => (
              <Season key={season.title}>
                ...
                <SeasonText>
                  <SeasonTitle as="h3">{season.title}</SeasonTitle>
